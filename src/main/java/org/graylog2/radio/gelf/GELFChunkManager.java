...
public class GELFChunkManager extends Thread {
            ...
            try {
                ...
                for (Map.Entry<String, Map<Integer, GELFMessageChunk>> message : chunks.entrySet()) {
                    ...
                    if (isComplete(messageId)) {
                        ...
                        dispatcher.messageReceived(chunksToByteArray(messageId));
    ...
    private boolean isComplete(String messageId) {
    ...
    private boolean isOutdated(String messageId) {
        if (!chunks.containsKey(messageId)) {
    ...
    private byte[] chunksToByteArray(String messageId) throws Exception {
        if (!chunks.containsKey(messageId)) {
    ...
    private boolean hasMessage(String messageId) {
    ...
    public void insert(byte[] payload) {
        insert(new GELFMessageChunk(payload));
    }
