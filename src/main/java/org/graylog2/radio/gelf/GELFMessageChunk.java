...
package org.graylog2.radio.gelf;
...
public final class GELFMessageChunk {
    ...
    public GELFMessageChunk(final byte[] source) {
        if (source.length < HEADER_TOTAL_LENGTH) {
    ...
    private String extractId() {
        if (this.id == null) {
            ...
            for (int i = 0; i < HEADER_PART_HASH_LENGTH; i++) {
                ...
                tmp = tmp.concat(Integer.toString((source[i+HEADER_PART_HASH_START] & 0xff) + 0x100, 16).substring(1));
            }
            this.id = tmp;
        }
    ...
    private int sliceInteger(final int start, final int length) {
        ...
        for (int i = 0; i < length; i++) {
            tmp = tmp.concat(Integer.toString(source[i+start]));
        }
        return Integer.parseInt(tmp);
    }

    private byte[] slice(final int cutOffAt) {
        final byte[] tmp = new byte[source.length-cutOffAt];

        int j = 0;
        for (int i = cutOffAt; i < source.length; i++) {
            tmp[j] = source[i];
            j++;
        }
